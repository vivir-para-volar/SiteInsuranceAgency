...
namespace InsuranceAgency
{
    public class MvcApplication : System.Web.HttpApplication
    {
        protected void Application_Start()
        {
            ...
            {
                MyIdentityRole newRole = new MyIdentityRole("Operator", "ќператоры могут только добавл€ть и измен€ть данные в системе");
                roleManager.Create(newRole);
            }

            if (!roleManager.RoleExists("User"))
            {
                MyIdentityRole newRole = new MyIdentityRole("User", "ѕользователи могут только смотреть свои данные в системе");
                roleManager.Create(newRole);
            }
        }
    }
